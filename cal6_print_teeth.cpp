...
int main() {
    ...
    try {
        ...
        zeroAxisPair(tic_tw_r, tic_cw_r, 1, -283000); // in and out
        zeroAxisPair(tic_tw_t, tic_cw_t, 1, -335287); // left and right 
        zeroAxisPair(tic_tw_z1, tic_tw_z2, tic_cw_z1, tic_cw_z2, 0, 26600); //was 19000 up and down
        ...
        int32_t vel9rpm = static_cast<int32_t>(245426 * 6.0 / 60.0); // check doc
        ...
        system(
            ...
            "--no-border --loop=inf "
            "--video-rotate=180 "
            "/home/jacob/Desktop/HeliCAL_Final/Videos/LVUDMA_teeth3_rebinned_36degps_intensity2xarray1final.mp4 &"
        );
        sleep(2);
        ...
        // turn on LED
        led.current(700); //Was 7500, leave at lower than 1000 when testing 
        led.PWM(255); // always leave 255
        // Monitor for SPACE to kill everything
        // 5) Multi-pass Z translation 9893413
        const vector<int32_t> steps = {9893413, -9893413};
        const int start_delay_ms = 0;
        const int delay_ms = 35000 + 0;
