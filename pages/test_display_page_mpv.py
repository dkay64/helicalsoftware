...
class JetsonController(QThread):
    def __init__(self, script_name):
        ...
        self.host = "192.168.0.116"  #Enter IP
        self.user = "jacob" #Enter user
        self.password = "helical" #passwd
    def run(self):
        try:
            ...
            # THE FIX: 
            # 1. pkill -f: Kills any old camera/rtsp processes
            # 2. sleep 1: Gives the hardware a second to reset
            # 3. run your script
            command = (
                "pkill -f start_camera_stream.sh; "
                "pkill -f v4l2-rtsp-server; "
                "sleep 1; "
                f"cd Desktop && nohup ./{self.script_name} > /dev/null 2>&1 &"
            )
